...
func (m *DeltaMonitor[ID]) PutEntity(id ID, data any, must bool) bool {
	...
	default:
		// channel is full
		log.Info("DeltaMonitor PutEntity channel is full, entity id %v, data: %v", id, data)
		if must {
			m.deltas <- item
			return true
		}
...
func (m *DeltaMonitor[ID]) SaveChangedDatas() {
	for _, v := range m.changes {
		log.Debug("collector size %s:%v", m.collName, v.CollectSize())
...
func (m *DeltaMonitor[ID]) saveEntityToDb(id ID, data any) error {
	defer func() {
		if err := recover(); err != nil {
			log.Error("save data to mongo panic, entity id %v, data:%v, err:%v\n%v", id, data, err, string(debug.Stack()))
		}
	}()
	if data == nil {
		return nil
	...
	if _, err := m.coll.UpdateOne(ctx, filter, data, opts); err != nil {
		log.Error("save data to mongo failed, entity id %v, data:%v, err:%v", id, data, err)
		return err
	}
	log.Debug("save data to mongo success, entity id %v, data:%v", id, data)
	return nil
}
...
func (m *DeltaMonitor[ID]) deleteEntityInDb(id ID) error {
	defer func() {
		if err := recover(); err != nil {
			log.Error("delete data to mongo panic, entity id %v, err:%v\n%v", id, err, string(debug.Stack()))
		}
	}()
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	...
	if _, err := m.coll.DeleteOne(ctx, filter); err != nil {
		log.Error("delete data to mongo failed, entity id %v, err:%v", id, err)
		return err
	}
	log.Debug("delete data to mongo success, entity id %v", id)
	return nil
}
...
func (m *DeltaMonitor[ID]) mustProcessEntity(id ID, data any) {
	...
	for {
		...
		if mongo.IsTimeout(err) || mongo.IsNetworkError(err) || err == mongo.ErrClientDisconnected {
			log.Info("DeltaMonitor (%s, %s) retry save data to mongo after %d seconds", m.dBName, m.collName, retryInterval)
